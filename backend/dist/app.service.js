...
let AppService = class AppService {
    ...
    async seedDatabase() {
        try {
            if (!(await this.dbAvailable()))
                return { ok: false, message: "database unavailable; using in-memory seed data" };
    ...
    listFacilities(query) {
        ...
        const result = facilities
            ...
            if (query.category && facility.category !== query.category)
                return false;
            if (query.wilaya && query.wilaya !== "All" && facility.wilaya !== query.wilaya)
                return false;
            if (query.providerType && query.providerType !== "All" && facility.providerType !== query.providerType)
                return false;
            if (maxBudget && facility.price > maxBudget)
                return false;
    ...
    addActivityContext(userId = "client-1", facilityId) {
        this.getFacility(facilityId);
        const set = this.activityContext.get(userId) ?? new Set();
        set.add(facilityId);
        this.activityContext.set(userId, set);
        return this.getActivityContext(userId);
    }
    removeActivityContext(userId = "client-1", facilityId) {
    ...
    createContact(body) {
        const contact = { id: `contact-${randomUUID()}`, status: "new", ...body };
    ...
    session(user) {
        ...
        return {
            user: safeUser,
            token: jwt.sign(safeUser, config.jwtSecret, { expiresIn: config.jwtExpiresIn }),
            refreshToken: jwt.sign({ sub: user.id, type: "refresh" }, config.jwtSecret, { expiresIn: config.jwtRefreshExpiresIn }),
        };
    }
    safeUser(user) {
        return {
